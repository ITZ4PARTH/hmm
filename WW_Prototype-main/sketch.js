...
var zombieGroup,WWGroup,rb,rbi
...
rbi  = loadImage ("assets/KAY-removebg-preview.png")
...
function enemy(){
  ...
  function rbe(){
    if(frameCount%50===0){
      ...
      //giving random x and y positions for zombie to appear
      rb = createSprite(Math.round(random(width/2+600,width/2+1000)),Math.round(random(height/2-600,width/2+200)),40,40)
    }
     rb.addImage(rbi)
     rb.scale = 0.15
     rb.velocityX = -3
     rb.debug= true
     rb.setCollider("rectangle",0,0,400,400)
